...
4. **`src/uncertainty.py`** — pooling ponderado por ROI (máscaras de disco) y utilidades de incertidumbre. Responde a la crítica de *dilución espacial* (disco óptico = 5–10% de la imagen). **Regla numérica dura (detectada en val_07, 23-jul-2026):** la KL/JSD sobre hidden states se computa SIEMPRE con `F.log_softmax` en **float64** — las *massive activations* de Gemma (10²–10³ en capas tardías) colapsan `softmax` float32 a ceros exactos y `softmax().log()` produce KL=inf. JSD = `scipy.spatial.distance.jensenshannon` **al cuadrado** (devuelve distancia, no divergencia). Computa además los baselines de igual costo (entropy, MSP, energy, TS) y el baseline 2× verbalized confidence (P5: parsing directo del número 0–100; u(x)=1−conf/100; solo sobre P1).
